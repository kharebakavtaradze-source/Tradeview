...
function getEtOffset(now) {
  ...
  const dstStart = (() => {
    ...
    return new Date(Date.UTC(year, 2, secondSun, 7));
  })();
  const dstEnd = (() => {
    const d = new Date(Date.UTC(year, 10, 1));
    const firstSun = 1 + (7 - d.getUTCDay()) % 7;
    return new Date(Date.UTC(year, 10, firstSun, 6));
  })();
...
function getMarketCountdown() {
  ...
  if (open) {
    const closeMs = new Date(Date.UTC(
      etNow.getUTCFullYear(), etNow.getUTCMonth(), etNow.getUTCDate(), 16, 0, 0
    )).getTime() + offsetMs;
    ...
    if (!isWeekend && etMinutes < OPEN) {
      ...
      let d = day;
      do { daysUntilOpen++; d = (d + 1) % 7; } while (d === 0 || d === 6);
    }
    const openMs = new Date(Date.UTC(
      etNow.getUTCFullYear(), etNow.getUTCMonth(), etNow.getUTCDate() + daysUntilOpen, 9, 30, 0
    )).getTime() + offsetMs;
...
function MarketContextBar({ marketTimer, regime, hypeStatus, hypeResults, livePrices, marketOpen }) {
  const regimeState = regime?.regime_state || regime?.state;
  const regimeColor = REGIME_COLORS[regimeState] || '#9ca3af';
  const hotTickers = (hypeStatus?.hot_tickers || []).slice(0, 8);
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        padding: '7px 14px', borderRight: '1px solid #1f2937',
        minWidth: 180,
      }}>
        <span style={{
      ...
      {regimeState && (
        <div style={{
          ...
          <span style={{ color: '#6b7280', textTransform: 'uppercase', letterSpacing: '0.07em' }}>Regime</span>
          <span style={{ color: regimeColor, fontWeight: 700, fontSize: 11 }}>
            {regimeState.replace(/_/g, ' ')}
          </span>
          {regime?.cycle_phase && (
            <span style={{ color: '#4b5563', fontSize: 10 }}>{regime.cycle_phase}</span>
          )}
        </div>
      )}
      ...
      {hotTickers.length > 0 && (
        <div style={{
          ...
          <span style={{ color: '#a78bfa', fontWeight: 700, letterSpacing: '0.05em' }}>HYPE</span>
          {hotTickers.map(t => (
            <span key={t} style={{ color: '#c4b5fd', fontFamily: 'monospace', fontSize: 11, fontWeight: 600 }}>{t}</span>
          ))}
          {hypeStatus?.total_divergences > 0 && (
            <span style={{ color: '#4b5563', fontSize: 10 }}>{hypeStatus.total_divergences} signals</span>
          )}
        </div>
      )}
      ...
      {marketOpen && liveCount > 0 && (
        <div style={{
          ...
          <span style={{ color: '#34d399', fontSize: 10 }}>Live prices · {liveCount} tickers · 30s</span>
        </div>
      )}
    </div>
  );
...
function ResultRow({ r, onClick, livePrice }) {
  ...
  return (
    ...
    >
      ...
      <td style={{ padding: '8px 8px', fontSize: 11, fontVariantNumeric: 'tabular-nums' }}>
        <span style={{ color: '#e5e7eb' }}>${fmt(price, 2)}</span>
        {changePct != null && (
          <span style={{ marginLeft: 4, fontSize: 10, color: changePct >= 0 ? '#34d399' : '#f87171' }}>
...
export default function DashboardPage() {
  ...
  // Market context — regime + hype, reload every 5 minutes
  const fetchContext = useCallback(async () => {
    try {
      const [regRes, statusRes, resultsRes] = await Promise.all([
    ...
    } catch { /* optional context — silent fail */ }
  }, []);

  useEffect(() => {
    fetchContext();
    const id = setInterval(fetchContext, 5 * 60 * 1000);
  ...
  // Live price overlay — every 30s during market hours
  ...
  const results = readyF ? rawResults.filter(r => r.readiness_tier === readyF) : rawResults;
  ...
  return (
    <>
      ...
      <MarketContextBar
        ...
        hypeResults={hypeResults}
        livePrices={livePrices}
        marketOpen={marketTimer.open}
      />
