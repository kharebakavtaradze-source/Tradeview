...
# ─── AI Analysis Layer tables (read-only advisory — never affects scan logic) ─

class AISignalAnalysis(Base):
    """
    Per-symbol AI analyst output for a specific scan run.
    Additive only — does not affect scores, tiers, or ranking.
    """
    __tablename__ = "ai_signal_analysis"

    id             = Column(Integer, primary_key=True, index=True)
    scan_id        = Column(Integer, nullable=True, index=True)    # FK → scans.id (soft)
    scanned_at     = Column(DateTime, nullable=True, index=True)   # denormalised for easy query
    symbol         = Column(String(10), nullable=False, index=True)
    verdict        = Column(String(30), nullable=True)             # STRONG_WATCH / WATCH / …
    quality_view   = Column(String(20), nullable=True)             # EARLY / CONFIRMED / …
    strengths_json = Column(Text, nullable=True)                   # JSON list
    weaknesses_json= Column(Text, nullable=True)                   # JSON list
    confirms_json  = Column(Text, nullable=True)                   # JSON list
    invalidates_json= Column(Text, nullable=True)                  # JSON list
...
class AIReviewReport(Base):
    """
    Daily / weekly / missed-opportunity / failure review reports.
    Generated by the AI Reviewer. Advisory only.
    """
    __tablename__ = "ai_review_reports"

    id                  = Column(Integer, primary_key=True, index=True)
    review_type         = Column(String(30), nullable=False, index=True)  # DAILY/WEEKLY/…
    review_date         = Column(String(10), nullable=False, index=True)  # YYYY-MM-DD
    summary             = Column(Text, nullable=True)
    what_worked_json    = Column(Text, nullable=True)    # JSON list
    what_failed_json    = Column(Text, nullable=True)    # JSON list
    missed_patterns_json= Column(Text, nullable=True)   # JSON list
    suggested_focus_json= Column(Text, nullable=True)   # JSON list
    confidence          = Column(String(10), nullable=True)
    data_quality        = Column(String(20), nullable=True)   # SUFFICIENT / SPARSE / MINIMAL
    created_at          = Column(DateTime, default=datetime.utcnow, index=True)


class AIInsight(Base):
    """
    Learning-layer insights and suggested experiments.
    Never auto-applied to production logic.
    """
    __tablename__ = "ai_insights"

    id                   = Column(Integer, primary_key=True, index=True)
    insight_type         = Column(String(30), nullable=False, index=True)  # PATTERN / …
    title                = Column(String(200), nullable=True)
    description          = Column(Text, nullable=True)
    evidence_json        = Column(Text, nullable=True)             # JSON list
...
# ─── AI Analysis Layer — CRUD functions ──────────────────────────────────────

async def save_ai_analyst_results(
    results: list[dict],
    scan_id: int | None = None,
    scanned_at: datetime | None = None,
) -> int:
    """
    Persist a batch of AI analyst outputs.
    Clears any prior rows for the same (scan_id or today's date) first so each
    run replaces the previous one.  Returns count of saved rows.
    """
    if not results:
        return 0
    today = (scanned_at or datetime.utcnow()).date()
    async with get_session_factory()() as session:
        # Prune rows from the same session (same scan_id or same day)
        if scan_id is not None:
            old = await session.execute(
                select(AISignalAnalysis).where(AISignalAnalysis.scan_id == scan_id)
            )
        else:
            cutoff = datetime.utcnow().replace(hour=0, minute=0, second=0, microsecond=0)
            old = await session.execute(
                select(AISignalAnalysis).where(AISignalAnalysis.created_at >= cutoff)
            )
        ...
        for r in results:
            row = AISignalAnalysis(
                scan_id         = scan_id,
                scanned_at      = ts,
                symbol          = r.get("symbol", ""),
                verdict         = r.get("verdict"),
                quality_view    = r.get("quality_view"),
                strengths_json  = json.dumps(r.get("strengths") or []),
                weaknesses_json = json.dumps(r.get("weaknesses") or []),
                confirms_json   = json.dumps(r.get("what_confirms") or []),
                invalidates_json= json.dumps(r.get("what_invalidates") or []),
...
def _ai_analysis_to_dict(row: AISignalAnalysis) -> dict:
    return {
        "id":             row.id,
        "scan_id":        row.scan_id,
        ...
        "symbol":         row.symbol,
        "verdict":        row.verdict,
        "quality_view":   row.quality_view,
        "strengths":      json.loads(row.strengths_json  or "[]"),
        "weaknesses":     json.loads(row.weaknesses_json or "[]"),
        "what_confirms":  json.loads(row.confirms_json   or "[]"),
        "what_invalidates": json.loads(row.invalidates_json or "[]"),
...
async def get_ai_analyst_latest(limit: int = 50) -> list[dict]:
    """Return the most recent AI analyst outputs (up to `limit`)."""
    async with get_session_factory()() as session:
        result = await session.execute(
            select(AISignalAnalysis)
            .order_by(AISignalAnalysis.created_at.desc())
            .limit(limit)
        )
        rows = result.scalars().all()
        return [_ai_analysis_to_dict(r) for r in rows]


async def get_ai_analyst_for_symbol(symbol: str) -> list[dict]:
    """Return AI analyst history for a specific symbol (last 10 analyses)."""
    async with get_session_factory()() as session:
        result = await session.execute(
            select(AISignalAnalysis)
            .where(AISignalAnalysis.symbol == symbol.upper())
            .order_by(AISignalAnalysis.created_at.desc())
            .limit(10)
        )
        rows = result.scalars().all()
...
async def save_ai_review(review: dict) -> int:
    """
    Persist one AI review report.  One row per (review_type, review_date) —
    upserts by deleting the existing row first.
    Returns the new row id.
    """
    review_date = (review.get("generated_at") or datetime.utcnow().isoformat())[:10]
    review_type = review.get("review_type", "DAILY")
    async with get_session_factory()() as session:
        old = await session.execute(
            select(AIReviewReport).where(
                (AIReviewReport.review_type == review_type) &
                (AIReviewReport.review_date == review_date)
            )
        )
        for row in old.scalars().all():
            await session.delete(row)
        ...
        row = AIReviewReport(
            ...
            what_worked_json     = json.dumps(review.get("what_worked") or []),
            what_failed_json     = json.dumps(review.get("what_failed") or []),
            missed_patterns_json = json.dumps(review.get("missed_patterns") or []),
            suggested_focus_json = json.dumps(review.get("suggested_focus") or []),
...
def _review_to_dict(row: AIReviewReport) -> dict:
    return {
        "id":               row.id,
        ...
        "missed_patterns":  json.loads(row.missed_patterns_json or "[]"),
        "suggested_focus":  json.loads(row.suggested_focus_json or "[]"),
        "confidence":       row.confidence,
...
async def get_ai_reviews(review_type: str | None = None, limit: int = 10) -> list[dict]:
    """Return recent AI review reports, optionally filtered by type."""
    async with get_session_factory()() as session:
        q = select(AIReviewReport).order_by(AIReviewReport.created_at.desc())
        if review_type:
            q = q.where(AIReviewReport.review_type == review_type)
        q = q.limit(limit)
...
async def save_ai_insights(insights: list[dict]) -> int:
    """Persist a batch of AI insights. Returns count saved."""
    if not insights:
        return 0
    async with get_session_factory()() as session:
        for ins in insights:
            row = AIInsight(
                insight_type         = ins.get("insight_type", "MODEL_NOTE"),
                title                = ins.get("title"),
                description          = ins.get("description"),
                evidence_json        = json.dumps(ins.get("evidence") or []),
                suggested_experiment = ins.get("suggested_experiment"),
...
def _insight_to_dict(row: AIInsight) -> dict:
    return {
        "id":                   row.id,
...
async def get_ai_insights(limit: int = 20) -> list[dict]:
    """Return the most recent AI insights."""
    async with get_session_factory()() as session:
        result = await session.execute(
            select(AIInsight)
